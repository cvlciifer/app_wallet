              ...
              children: [
                ...
                Column(
                  children: state.previewMonths.map((d) {
                    ...
                    String monthLabelRaw = (d.year == DateTime.now().year && d.month == DateTime.now().month) ? 'Mes actual' : DateFormat('MMMM yyyy', 'es').format(d);
                      ...
                      child: IncomePreviewTile(
                        ...
                        onTap: () async {
                          final saved = await Navigator.of(context).push<bool>(
                            MaterialPageRoute(
                              builder: (_) => IngresosImprevistosPage(initialMonth: monthDate, initialImprevisto: imprevisto),
                            ),
                          );
                              ...
                              child: Padding(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    CustomTextField(controller: fijoCtrl, label: 'Ingreso mensual', keyboardType: TextInputType.number, inputFormatters: [MaxAmountFormatter(maxDigits: MaxAmountFormatter.kEightDigits, maxAmount: MaxAmountFormatter.kEightDigitsMaxAmount, onAttemptOverLimit: () { if (mounted) { /* ignore UI here */ } }), CLPTextInputFormatter()]),
                                    AwSpacing.s6,
                                    CustomTextField(controller: impCtrl, label: 'Imprevisto (opcional)', keyboardType: TextInputType.number, inputFormatters: [MaxAmountFormatter(maxDigits: MaxAmountFormatter.kEightDigits, maxAmount: MaxAmountFormatter.kEightDigitsMaxAmount, onAttemptOverLimit: () { if (mounted) { /* ignore UI here */ } }), CLPTextInputFormatter()]),
                                    AwSpacing.s12,
                                    Row(
                        ...
                        onDelete: () async {
                          final confirm = await showDialog<bool>(
                            ...
                            builder: (ctx) => AlertDialog(
                              ...
                              content: AwText.normal('¿Eliminar ingreso fijo y imprevisto de ${DateFormat('MMMM yyyy', 'es').format(monthDate)}?', size: AwSize.s14, color: AwColors.modalGrey),
                              actions: [
                                TextButton(onPressed: () => Navigator.of(ctx).pop(false), child: const Text('Cancelar')),
                          ...
                          if (confirm == true) {
                            ...
                            if (ok) {
                              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Ingreso eliminado')));
                              await ctrl.loadLocalIncomes();
                            } else {
                              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Error eliminando ingreso')));
                            }
                          }
                        },
                      ),
