...
class SyncService {
  ...
  SyncService(
      {required this.localCrud,
      required this.firestore,
      required this.userEmail});

  Future<void> initializeLocalDbFromFirebase() async {
    if (_initializing) {
      log('initializeLocalDbFromFirebase: ya se está ejecutando, esperando a que termine la inicialización existente');
      try {
  ...
  Future<void> syncPendingChanges() async {
    ...
    try {
      ...
      final creates = pendingExpenses
          .where((e) => e.syncStatus == SyncStatus.pendingCreate)
          .toList();
      final updates = pendingExpenses
          .where((e) => e.syncStatus == SyncStatus.pendingUpdate)
          .toList();
      final deletes = pendingExpenses
      ...
      try {
        ...
        if (pendingIncomes.isNotEmpty) {
          final creates = pendingIncomes
              .where((r) =>
                  (r['sync_status'] as int) == SyncStatus.pendingCreate.index)
              .toList();
          final updates = pendingIncomes
              .where((r) =>
                  (r['sync_status'] as int) == SyncStatus.pendingUpdate.index)
              .toList();
          final deletesIn = pendingIncomes
              .where((r) =>
                  (r['sync_status'] as int) == SyncStatus.pendingDelete.index)
              .toList();
          ...
          for (final row in creates) {
            await _safeRun(() async {
              final email = await _resolveEmail();
              if (email.isEmpty)
                throw Exception('No user email to sync create');
              final docRef = firestore
                  .collection('usuarios')
                  .doc(email)
                  .collection('ingresos')
              ...
              await docRef.set({
                'fecha':
                    Timestamp.fromMillisecondsSinceEpoch(row['fecha'] as int),
                'ingreso_fijo': row['ingreso_fijo'] ?? 0,
                'ingreso_imprevisto': row['ingreso_imprevisto'] ?? 0,
                'ingreso_total': row['ingreso_total'] ??
                    ((row['ingreso_fijo'] ?? 0) +
                        (row['ingreso_imprevisto'] ?? 0)),
                'id': row['id'],
              });
              await localCrud.updateIncomeSyncStatus(
                  row['id'] as String, SyncStatus.synced);
          ...
          for (final row in updates) {
            await _safeRun(() async {
              final email = await _resolveEmail();
              if (email.isEmpty)
                throw Exception('No user email to sync update');
              final docRef = firestore
                  .collection('usuarios')
                  .doc(email)
                  .collection('ingresos')
          ...
          for (final row in deletesIn) {
            await _safeRun(() async {
              final email = await _resolveEmail();
              if (email.isEmpty)
                throw Exception('No user email to sync delete');
              final docRef = firestore
                  .collection('usuarios')
                  .doc(email)
                  .collection('ingresos')
  ...
  Future<void> createExpense(Expense expense,
    ...
    await localCrud
        .insertExpense(expense.copyWith(syncStatus: SyncStatus.pendingCreate));
  ...
  Future<void> updateExpense(Expense expense,
    ...
    if (hasConnection) {
      ...
      if (email.isNotEmpty) {
        ...
        await localCrud
            .updateExpense(expense.copyWith(syncStatus: SyncStatus.synced));
        return;
      }
      await localCrud.updateExpense(
          expense.copyWith(syncStatus: SyncStatus.pendingUpdate));
    } else {
      await localCrud.updateExpense(
          expense.copyWith(syncStatus: SyncStatus.pendingUpdate));
    }
